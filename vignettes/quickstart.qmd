...
## Package Dumps for LLM Tools

`create_rdd()` writes package docs/code/vignettes to a single text artifact that
works well for LLM-assisted CLI workflows.

```{r}
library(rdstools)

# Installed package
out <- create_rdd("pkgdown")
out
...
- Package names preserve dots in output files (`R.utils` -> `R.utils.txt`)
- Installed package names use local installed files
- Uninstalled names are fetched from CRAN
- Local directories/archives are read directly
